...
def test_backup_is_sent_as_a_document(app, db):
    ...
    db.snapshot.assert_called_once()
...
def test_the_document_is_named_after_the_moment(app, db):
    ...
    assert re.fullmatch(r"leistungsbot-backup-\d{8}-\d{6}\.sqlite", name)
...
def test_the_snapshot_does_not_stay_on_disk(app, db, tmp_path):
    support.send_command(app, "/backup")
...
def test_a_failing_snapshot_is_reported(app, db):
