...
from leistungsbot import Commands, leistungs_config as lc
...
async def alive(update: Update, context: LeistungsbotContext) -> int:
    ...
    await context.bot.send_message(update.effective_chat.id, "Wos brauchst denn schowieder? Da papa hüft da eh ...")
    return ConversationHandler.END
...
async def version(update: Update, context: LeistungsbotContext) -> int:
    ...
    await context.bot.send_message(update.effective_chat.id, f"LeistungsBot - {_version.__version__}")
...
async def message(update: Update, context: LeistungsbotContext) -> int:
    ...
    await context.bot.send_message(update.effective_chat.id, "Wos soll i sogen?")

    return ConversationState.MESSAGE

async def message_send_message(update: Update, context: LeistungsbotContext) -> int:
    if update.effective_chat is None:
        print("No effective chat in leistungpoll", file=sys.stderr)
        return ConversationHandler.END

    if update.message is None or update.message.text is None:
        await context.bot.send_message(update.effective_chat.id, "Du soist ma nochricht schicken!")
        return ConversationState.MESSAGE

    await context.bot.send_message(lc.config["leistungschat_id"], update.message.text)
