...
class LeistungsBot(object):
    def __init__(self) -> None:
        ...
        def callback_query(call):
            try:
                ...
                if cmd == 'search':
                    ...
                    elif self.bot.get_state(call.from_user.id, call.message.chat.id) == LeistungsState.genericLeistungsmessage.name:
                        self.process_generic_leistungsmessage(call.message.reply_to_message, val)
                elif cmd == 'closed':
                    self.bot.reply_to(call.message, f'Der Poll is scho closed. Willst wirklich on den reminden?',
                                reply_markup=self.helper.confirm_leistungstag_button(val))
                elif cmd == 'no':
                    self.process_generic_leistungsmessage(call.message.reply_to_message, val)
                elif cmd == 'poll_date':
                    if val:
        ...
        def pollNow(message: telebot.types.Message):
            try:
                leistungstage = None
                self.bot.set_state(message.from_user.id, LeistungsState.remindePoll, message.chat.id)
                ...
                if len(command_parts) == 1:
                    ...
                    try:
                        ...
                        target_date = None
                        self.bot.send_message(message.chat.id, f'Soi des a Datum sei? Schick ma wonn donn sowos wie {datetime.now().date().isoformat()}')
                        self.bot.reply_to(message, 'An welchen Poll wüst reminden?',
                                reply_markup=self.helper.open_polls_button())
                    ...
                    if target_date is not None:
                        successful = self.try_remind_to_leistungstag_on_a_specific_date(message, target_date)
                        if not successful:
                            self.bot.reply_to(message, 'An welchen Poll wüst reminden?',
                                    reply_markup=self.helper.open_polls_button())
        ...
        def message_handler(message: telebot.types.Message):
            try:
                ...
                if len(command_parts) < 2:
                    self.bot.reply_to(message, 'Jo, do muast jetzt scho dazuaschreiben wost willst. Probiers numoi')
                    return

                self.bot.set_state(message.from_user.id, LeistungsState.genericLeistungsmessage, message.chat.id)

                self.bot.reply_to(message, 'Wüst des auf irgend an poll replyen?',
        ...
        def new_msg(message):
            try:
                if 'nude' in message.text:
                    if message.chat.type != 'private':
                        if (datetime.now() - self.last_text_nudes) > timedelta(days=1):
    ...
    def process_closepoll(self, message, leistungstag_key):
        ...
        self.bot.unpin_chat_message(config['leistungschat_id'], leistungstag['poll_id'])
        self.bot.send_message(
    ...
    def process_generic_leistungsmessage(self, original_message: telebot.types.Message, leistungstag_id):
        ...
        if leistungstag_id is not None:
            ...
            self.bot.send_message(config['leistungschat_id'], msg, reply_to_message_id=leistungstag['poll_id'])
        else:
            self.bot.send_message(config['leistungschat_id'], msg)

        print(f'User {original_message.from_user.username} sent generic message "{msg}"')

        self.bot.delete_state(original_message.from_user.id, original_message.chat.id)
    ...
    def try_remind_to_leistungstag_on_a_specific_date(self, message: telebot.types.Message, target_date: date) -> bool:
        ...
        if target_date < datetime.now().date():
            self.bot.send_message(message.chat.id, 'Für des is zu spät zum reminden. Suach da wos ondares aus ...')
            return False
        ...
        if leistungstage is None:
            self.bot.send_message(message.chat.id, 'An dem Tog is ka Leistungstog. Suach da wos ondares aus ...')
            return False

        if len(leistungstage) == 1:
            leistunstag = leistungstage[0]
            location_name = self.helper.db.getLocationName(leistunstag['location'])

            if leistunstag['closed'] == 1:
                self.bot.reply_to(message, f'Leistungstag an dem Tog is bei {location_name}, aber da Poll is scho geclosed. Willst wirklich on den reminden?',
                                reply_markup=self.helper.confirm_leistungstag_button(leistunstag['key']))
            else:
                self.bot.reply_to(message, f'Leistungstag an dem Tog is bei {location_name}. Willst on den reminden?',
                                reply_markup=self.helper.confirm_leistungstag_button(leistunstag['key']))

        else:
            self.bot.send_message(message.chat.id, f'Ein wöd Tog. Do san sogor mehrere Leistungstoge!')
            self.bot.reply_to(message, 'An welchen Poll wüst reminden?',
