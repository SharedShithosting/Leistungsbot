    ...
    def polls_button(self, leistungstage, additional_button=False):
        ...
        if leistungstage:
            for leistungstag in leistungstage:
                callback_key = '🍻closed' if leistungstag["closed"] == 1 else '🍻open'
                markup.add(InlineKeyboardButton(
                    self.db.getLocationName(leistungstag['location']),
                    callback_data=json.dumps({callback_key: leistungstag['key']})))
        if additional_button:
            markup.add(InlineKeyboardButton('Nö', callback_data=json.dumps({'🍻no': None})))
        return markup
    ...
    def send_leistungstag(self, chat_id, location: str, type: LeistungsTyp = LeistungsTyp.NORMAL, date: datetime = None, dry_run: bool = True):
        ...
        if dry_run:
            ...
            self.db.addLeistungsTag(
                date, location, poll_message.message_id, venue_id.message_id, int(type))
